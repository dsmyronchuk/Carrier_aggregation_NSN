...
user_name = input('Введите полный нейм БС:')
user_MRBTS = int(input('Введите MRBTS БС:'))
...
strDistName1 = f'PLMN-PLMN/MRBTS-{user_MRBTS}LNBTS-{user_MRBTS}LNCEL-{Cells1800}/CAREL-1'
strDistName2 = f'PLMN-PLMN/MRBTS-{user_MRBTS}LNBTS-{user_MRBTS}LNCEL-{Cells2600}/CAREL-1'


def create_xml():
    xml = ET.Element('raml', version='2.0', xmlns='raml20.xsd')
    cm_data = ET.SubElement(xml, 'cmData', xmlns="", type='plan', scope='changes', name=user_name)
    ...
    if len(Cells1800) > 0 and len(Cells2600) > 0:
        while x < len(Cells1800):
            while x1 < len(Cells2600):
                ...
                managed_object = ET.SubElement(cm_data, 'managedObject', CAREL, version="xL20B_2003_002", distName=
                                               f'PLMN-PLMN/MRBTS-{user_MRBTS}/LNBTS-{user_MRBTS}/LNCEL-{Cells1800[x]}'
        ...
        while x < len(Cells2600):
            while x1 < len(Cells1800):
                ...
                managed_object = ET.SubElement(cm_data, 'managedObject', CAREL, version="xL20B_2003_002", distName=
                                               f'PLMN-PLMN/MRBTS-{user_MRBTS}/LNBTS-{user_MRBTS}/LNCEL-{Cells2600[x]}'
    ...
    if len(Cells900) > 0 and len(Cells1800) > 0:
        ...
        while x2 < len(Cells900):
            while x < len(Cells1800):
                ...
                managed_object = ET.SubElement(cm_data, 'managedObject', CAREL, version="xL20B_2003_002", distName=
                                               f'PLMN-PLMN/MRBTS-{user_MRBTS}/LNBTS-{user_MRBTS}/LNCEL-{Cells900[x2]}'
                                               f'/CAREL-{n_carel}', operation="create")
                defaults = ET.SubElement(managed_object, 'defaults', name="System")
                p1 = ET.SubElement(managed_object, 'p', name="lcrId")
    ...
    managed_object = ET.SubElement(cm_data, 'managedObject', version="xL20B_2003_002", distName=f'PLMN-PLMN/MRBTS-'
                                   f'{user_MRBTS}/LNBTS-{user_MRBTS}/CADPR-0', id="41429697", operation="create")
    defaults = ET.SubElement(managed_object, 'defaults', name="System")
